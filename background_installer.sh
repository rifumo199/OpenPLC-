...
function install_matiec {
    ...
    (
        cd "$OPENPLC_DIR/utils/matiec_src" || fail "matiec_src directory not found"
        autoreconf -i && ./configure && make && cp ./iec2c "$OPENPLC_DIR/webserver/"
    ) || fail "Error compiling MatIEC"
}

function install_st_optimizer {
    echo "[ST OPTIMIZER]"
    (
        cd "$OPENPLC_DIR/utils/st_optimizer_src" || fail "st_optimizer_src directory not found"
        g++ st_optimizer.cpp -o "$OPENPLC_DIR/webserver/st_optimizer"
    ) || fail "Error compiling ST Optimizer"
}

function install_glue_generator {
    echo "[GLUE GENERATOR]"
    (
        cd "$OPENPLC_DIR/utils/glue_generator_src" || fail "glue_generator_src directory not found"
        g++ -std=c++11 glue_generator.cpp -o "$OPENPLC_DIR/webserver/core/glue_generator"
    ) || fail "Error compiling Glue Generator"
}
